...
//get total carbon emissions calculated in the backend
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8080/api/environmental-impact/carbon"
        );
  ...
  return (
    <div className="carbon">
      ...
      <div className="carbon__container">
        <p className="carbon__number">
          {" "}
          <img className="carbon__icon" src={carbon} alt="cloud" />
          {totalC02} kg{" "}
        </p>
      </div>
    </div>
