...
function WasteOverTime() {
  ...
  //show waste item trends over time with items grouped together by the date they were thrown away
  //include message based on the trend of the line
  useEffect(() => {
    const getData = async () => {
      try {
  ...
  //group data by the date rather than timestamps
  const aggregateByDay = (rawData) => {
    ...
    return Array.from(aggregatedData.entries()).map(([date, count]) => [
      date,
      count,
    ]);
  };
